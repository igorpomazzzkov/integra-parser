...
select_crosses = """SELECT cid, id, brand_art, brand FROM cross_articles WHERE id NOT IN (SELECT DISTINCT cross_id FROM cross_laximo) AND id NOT IN (SELECT cross_id FROM cross_laximo_not_found WHERE response_code = 200) AND supplier != 3"""
...
insert_cross_laximo_not_found = """INSERT INTO cross_laximo_not_found(cross_id, response_code) VALUES(%s, %s)"""